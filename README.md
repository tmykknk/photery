...
5. `http://localhost:3000/` に戻り、画像一覧が表示されるか確認
...
- `app/login/page.tsx`: 閲覧用パスワード入力画面です。`/api/auth` に送信して認証状態を作ります。
...
- `app/api/sync/route.ts`: Google Drive APIで対象フォルダ内の画像を取得し、`drive_images` テーブルへupsertします。
...
- `app/components/gallery-utils.ts`: 日付表示など、ギャラリー用の小さなユーティリティ関数を定義します。
...
- `next.config.ts`: Next.js設定です。現在は追加設定を持たない最小構成です。
...
### Package / Workspace
- `package.json`: 依存関係と `pnpm dev` / `pnpm build` / `pnpm lint` などのスクリプトを管理します。
